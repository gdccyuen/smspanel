...
def test_csrf_protection_initialized():
    ...
    assert app.config.get("WTF_CSRF_ENABLED")
...
def test_login_rejects_invalid_csrf():
    ...
    with app.test_client() as client:
        response = client.post(
            "/login",
            data={"username": "SMSadmin", "password": "SMSpass#12", "csrf_token": "invalid-token"},
        )
...
def test_all_forms_have_csrf_token():
    ...
    templates_dir = os.path.join(
        os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "src", "smspanel", "templates"
    )
    ...
    for root, _, files in os.walk(templates_dir):
        for fname in files:
            if fname.endswith(".html"):
                ...
                if "<form" in content:
                    # Check for inline CSRF token (not relying on base template)
                    has_inline_csrf = 'name="csrf_token"' in content or "csrf_token()" in content
                    assert has_inline_csrf, f"Template {fname} has form but no CSRF token"
